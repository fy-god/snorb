...
class DeviceController(private val context: Context? = null) {
     ...
     * 执行 shell 命令并返回输出
     */
    fun shell(command: String): String = exec(command)
    ...
    fun clickByTarget(target: SkillTarget): Boolean {
        ...
        if (a11y != null) {
            val clicked = a11y.clickByTarget(target)
            if (clicked) return true
        }
        if (target.fallbackX != null && target.fallbackY != null) {
            val (screenW, screenH) = getScreenSize()
            val x = mapTargetCoordinate(target.fallbackX, screenW)
            val y = mapTargetCoordinate(target.fallbackY, screenH)
            return gestureTap(x, y)
        }
        println("[DeviceController] clickByTarget failed: text=${target.text}, class=${target.className}, desc=${target.contentDesc}")
        return false
    }

    fun focusByTarget(target: SkillTarget): Boolean {
        val focused = AutomationAccessibilityService.instance?.focusByTarget(target) == true
        println("[DeviceController] focusByTarget focused=$focused target=$target")
        return focused
    }

    fun setTextByAccessibility(target: SkillTarget, text: String): Boolean {
        val set = AutomationAccessibilityService.instance?.setTextByTarget(target, text) == true
        println("[DeviceController] setTextByAccessibility set=$set text=${text.take(20)}")
        return set
    }
    ...
    fun gestureTap(x: Int, y: Int): Boolean {
        val a11y = AutomationAccessibilityService.instance
        if (a11y != null) {
            val tapped = a11y.gestureTap(x, y)
    ...
    fun dumpActiveWindowHash(): String {
        return AutomationAccessibilityService.instance?.activeWindowHash().orEmpty()
    }

    private fun mapTargetCoordinate(value: Int, screenMax: Int): Int {
        return if (value < 1000) value * screenMax / 1000 else value.coerceAtMost(screenMax)
    }
    ...
    suspend fun screenshotWithFallback(): ScreenshotResult = withContext(Dispatchers.IO) {
        try {
            ...
            delay(300)  // screencap 通常 0.2-0.3s 完成
    ...
    fun openApp(appNameOrPackage: String) {
        ...
        // 优先使用 PackageManager Intent 启动 (不需要 shell 权限)
        val launchIntent = context?.packageManager?.getLaunchIntentForPackage(finalPackage)
        if (launchIntent != null) {
            launchIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            context?.startActivity(launchIntent)
            println("[DeviceController] openApp via Intent: $appNameOrPackage -> $finalPackage ✅")
        } else {
            // 降级: monkey 命令
            val result = exec("monkey -p $finalPackage -c android.intent.category.LAUNCHER 1 2>/dev/null")
            println("[DeviceController] openApp via monkey: $appNameOrPackage -> $finalPackage, result: $result")
        }
    }
