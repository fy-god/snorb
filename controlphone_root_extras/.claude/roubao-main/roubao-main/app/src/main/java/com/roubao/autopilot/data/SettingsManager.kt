...
const val DEFAULT_AGENT_MODEL = "deepseek-chat"
private const val AGENT_PROVIDER_ID = "agent_runtime"
...
data class AppSettings(
    ...
    val agentApiKey: String get() = providerConfigs[AGENT_PROVIDER_ID]?.apiKey.orEmpty()
    val agentBaseUrl: String get() = providerConfigs[AGENT_PROVIDER_ID]?.customBaseUrl.orEmpty()
    val agentModel: String get() = providerConfigs[AGENT_PROVIDER_ID]?.model?.ifEmpty { DEFAULT_AGENT_MODEL } ?: DEFAULT_AGENT_MODEL
...
class SettingsManager(context: Context) {
    ...
    private fun loadSettings(): AppSettings {
        ...
        // 加载当前选中的服务商
        var currentProviderId = prefs.getString("current_provider_id", ApiProvider.ALIYUN.id) ?: ApiProvider.ALIYUN.id
        ...
        providerConfigs[AGENT_PROVIDER_ID] = loadProviderConfig(AGENT_PROVIDER_ID).let {
            if (it.model.isBlank()) it.copy(model = DEFAULT_AGENT_MODEL) else it
        }
        ...
        if (oldApiKey != null || oldModel != null) {
            ...
            currentProviderId = oldProviderId
    ...
    fun getAgentApiKey(): String = _settings.value.agentApiKey

    fun setAgentApiKey(value: String) {
        updateAgentConfig { it.copy(apiKey = value) }
    }

    fun getAgentBaseUrl(): String = _settings.value.agentBaseUrl

    fun setAgentBaseUrl(value: String) {
        updateAgentConfig { it.copy(customBaseUrl = value) }
    }

    fun getAgentModel(): String = _settings.value.agentModel

    fun setAgentModel(value: String) {
        updateAgentConfig { it.copy(model = value.ifBlank { DEFAULT_AGENT_MODEL }) }
    }

    private fun updateAgentConfig(update: (ProviderConfig) -> ProviderConfig) {
        val currentConfig = _settings.value.providerConfigs[AGENT_PROVIDER_ID] ?: ProviderConfig(model = DEFAULT_AGENT_MODEL)
        val newConfig = update(currentConfig)
        saveProviderConfig(AGENT_PROVIDER_ID, newConfig)
        val newConfigs = _settings.value.providerConfigs.toMutableMap()
        newConfigs[AGENT_PROVIDER_ID] = newConfig
        _settings.value = _settings.value.copy(providerConfigs = newConfigs)
    }
