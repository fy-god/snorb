...
import com.roubao.autopilot.agent.multi.AgentApiConfig
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 设置 BLE AI 指令回调 (ESP32 发来的复杂指令 → 触发 AI Agent)
        App.getInstance().onBleAICommand = { query ->
            val s = settingsManager.settings.value
            val agentConfig = AgentApiConfig.from(settingsManager)
            runAgent(
                instruction = query,
    ...
    private fun handleDebugRunIntent(startIntent: Intent?) {
        if (!BuildConfig.DEBUG) return
        val shouldAutoRun = startIntent?.getBooleanExtra("autoRun", false) == true ||
            startIntent?.getBooleanExtra("macroSelfTest", false) == true
        if (!shouldAutoRun) return

        val instruction = startIntent?.getStringExtra("instruction")
            ?: startIntent?.getStringExtra("macroInstruction")
            ?: "wx filehelper 111"
        if (instruction.isBlank()) return

        Log.d(TAG, "Debug macro self-test requested: $instruction")
        lifecycleScope.launch {
            ...
            runAgent(
                ...
                model = "",
                maxSteps = 8,
                providerId = "macro_debug",
                forceLocalMacroOnly = true
            )
        }
    }
                        ...
                        when (screen) {
                            Screen.Home -> {
                                ...
                                HomeScreen(
                                    ...
                                    onSaveWorkflow = {
                                        val actions = agent?.state?.value?.lastActions ?: emptyList()
                                        if (actions.isEmpty()) return@HomeScreen
                                        val instruction = agent?.state?.value?.instruction ?: "未命名"
                                        val traceStore = com.roubao.autopilot.macro.store.RecordedTraceStore(this@MainActivity)
                                        lifecycleScope.launch {
                                            val traceId = traceStore.savePendingActionTrace(
                                                instruction = instruction,
                                                appPackage = null,
                                                actions = actions,
                                                reason = "legacy action list has no before/after UI snapshots"
                                            )
                                            Toast.makeText(
                                                this@MainActivity,
                                                "\u5df2\u4fdd\u5b58\u4e3a\u5f85\u5904\u7406\u5f55\u5236: $traceId",
                                    ...
                                    skillHints = remember {
                                        App.getInstance().fragmentLibrary?.getTemplateHints()?.map {
                                            (trigger, desc) -> com.roubao.autopilot.ui.screens.SkillHint(trigger, desc)
                                        } ?: emptyList()
                                    }
                                )
                            }
    ...
    private fun runAgent(
        ...
        providerId: String = "",
        forceLocalMacroOnly: Boolean = false
    ) {
        ...
        val requiresApiKey = !forceLocalMacroOnly && providerId != "mai_ui"
        ...
        // 检查悬浮窗权限
        if (!forceLocalMacroOnly && !Settings.canDrawOverlays(this)) {
