        ...
        for (param in config.params) {
            when (param.name) {
                ...
                "message", "content", "prompt" -> {
                    ...
                    val messagePatterns = if (!contact.isNullOrBlank()) {
                        listOf(
                            "给${Regex.escape(contact)}发(.+)",
                            "给${Regex.escape(contact)}发送(.+)",
                            "跟${Regex.escape(contact)}说(.+)",
                            "告诉${Regex.escape(contact)}(.+)"
                        )
                    } else {
                        listOf("发(.+)", "发送(.+)", "说(.+)")
                    }
                    val extracted = messagePatterns.firstNotNullOfOrNull { pattern ->
                        Regex(pattern).find(query)?.groupValues?.getOrNull(1)?.trim()
                    }
                    params[param.name] = extracted?.takeIf { it.isNotEmpty() } ?: query
                }
                "time" -> {
                    // 提取时间
                    val patterns = listOf(
